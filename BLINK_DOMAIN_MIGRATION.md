# Domain Migration: mishmesh.ai → blinkworld.com

Pasquale bought blinkworld.com (the BLINK universe domain). This is now the primary domain. mishmesh.ai stays as a 301 redirect to blinkworld.com.

## Task

Find and update ALL references from `mishmesh.ai` to `blinkworld.com` in user-facing surfaces:

### Must update
- `src/app/layout.tsx` — `metadataBase`, `openGraph.url`, any URL refs
- `src/app/sitemap.ts` — `BASE` constant
- `src/middleware.ts` — CORS allowlist (add `blinkworld.com` + `www.blinkworld.com` — KEEP mishmesh.ai entries since we're 301'ing from it)
...
- `src/app/support/page.tsx` — `support@mishmesh.ai` → `support@blinkworld.com`, and any "mishmesh.ai/privacy" → "blinkworld.com/privacy"
- `src/app/api/auth/create-wallet/route.ts` — if there's `@wallet.mishmesh.ai`, change to `@wallet.blinkworld.com`
...
If `next.config.js` doesn't already have it: add a redirect block so requests to mishmesh.ai paths get a 301 to blinkworld.com (Vercel will route both domains here):
...
- One commit: `feat(domain): migrate primary domain to blinkworld.com with mishmesh.ai 301 redirect`
