...
function usdValue(amount: number, currency: OrbCurrency): number {
...
function OrbFragments({ rarity, size }: { rarity: OrbRarity; size: number }) {
  ...
  const gradMap: Record<OrbRarity, string> = {
    ...
    Rare: "linear-gradient(135deg, #93c5fd, #1e3a8a)",
    Legendary: "linear-gradient(135deg, #fde68a, #92400e)",
  };
...
function Confetti({ color }: { color: string }) {
  const pieces = useMemo(() =>
    Array.from({ length: 30 }, (_, i) => ({
      ...
      delay: Math.random() * 2,
      dur: 2 + Math.random() * 3,
      color: [color, C.primary, C.gold, "#fff", "#14F195"][i % 5],
    })), [color]
  );
...
export default function CrackPage() {
  ...
  const usd = usdValue(amount, currency);
  ...
  const callCrackAPI = useCallback(async () => {
    ...
    const orbLat = orb.latitude ?? 0;
    const orbLng = orb.longitude ?? 0;
    ...
    try {
      ...
      if (!res.ok || data.error) {
        setCrackError(data.error ?? "Failed to crack orb.");
        return;
      }
  ...
  const handleTap = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      /* After explosion clears (500ms) -> Phase 4: REVEAL */
      setTimeout(() => {
        phaseRef.current = "REVEAL";
        setPhase("REVEAL");
        setShowConfetti(true);
        /* Start count-up after a tiny delay */
  ...
  const handleConfirm = useCallback(() => {
    ...
    /* Simulate confirmation delay (or wait for real result) */
    const checkConfirm = () => {
      if (crackResult || crackError) {
    ...
    return (
      <div style={{
        ...
        <div style={{ textAlign: "center" }}>
          ...
          <p style={{ color: C.muted, fontSize: 14, marginTop: 16 }}>Loading orb...</p>
        </div>
      </div>
    );
  }
  ...
  if (!orb) {
    return (
      <div style={{
        ...
        <div style={{ color: C.text, fontSize: 20, fontWeight: 700 }}>Orb not found</div>
          ...
          {rarity !== "Common" && (
            <div style={{
              ...
              <span style={{ color: C.gold, fontSize: 12, fontWeight: 700, letterSpacing: "0.05em" }}>
                {rarity === "Legendary" ? "LEGENDARY" : "RARE"} DROP
              </span>
            </div>
          )}
            ...
            <div style={{ animation: "fadeIn 0.4s ease-out" }}>
              ...
              {crackResult?.tx_hash && (
                <div style={{
                  ...
                  tx: {truncateHash(crackResult.tx_hash)}
                </div>
              )}
              ...
              <div style={{ marginTop: 28 }}>
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 50,
                    border: "none",
                    background: `linear-gradient(135deg, ${C.primary}, #14F195)`,
                    color: "#fff",
                    fontSize: 17,
                    fontWeight: 800,
