...
type FilterKey = 'All Orbs' | 'Nearby' | 'Stealth' | 'In Flight';
const FILTERS: FilterKey[] = ['All Orbs', 'Nearby', 'Stealth', 'In Flight'];

const ACCENT = '#14F195';
const PRIMARY = '#9945FF';
const GOLD = '#F59E0B';
const BG = '#0A0A0F';
const TEXT = '#F9FAFB';
const MUTED = '#9CA3AF';
        ...
        return (
          <button
            ...
            style={{
              ...
              padding: '7px 16px',
              borderRadius: 20,
              border: `1.5px solid ${active ? ACCENT : GLASS_BORDER}`,
              background: active
    ...
    <div style={{
      ...
      <button
        ...
        style={{
          ...
          background: gpsActive ? ACCENT + '18' : 'rgba(239,68,68,0.12)',
        }}
        title={gpsActive ? 'Recenter on my location' : 'GPS off — tap to retry'}
      >
      ...
      {/* Nearby toggle */}
      <button
        onClick={() => onSetFilter(nearbyActive ? 'all' : 'nearby')}
        style={{
          ...btnStyle,
          flexDirection: 'column',
          gap: 2,
        ...
        title={nearbyActive ? 'Show all orbs' : 'Show nearby only'}
      >
        <NearbyIcon />
    ...
    <div style={{
      ...
      <a
        ...
        style={{
          ...
          padding: '12px 32px',
          borderRadius: 24,
          background: `linear-gradient(135deg, #9945FF, #8B5CF6)`,
          color: '#fff',
          fontSize: 15,
          fontWeight: 700,
          textDecoration: 'none',
          cursor: 'pointer',
          boxShadow: `0 4px 24px ${PRIMARY}66, 0 0 40px ${PRIMARY}22`,
          pointerEvents: 'auto',
...
export default function HuntPage() {
  ...
  const [activeFilter, setActiveFilter] = useState<FilterKey>('All Orbs');
      ...
      .on(
        ...
        (payload) => {
          if (payload.eventType === 'INSERT') {
            ...
            if (o.status === 'pending' || o.status === 'claimed') {
              setAllOrbs((prev) => [...prev, o]);
            }
          } else if (payload.eventType === 'UPDATE') {
            const o = payload.new as Orb;
            setAllOrbs((prev) =>
